...
pub fn provision(paths: &[PathBuf]) -> Result<HashSet<PathBuf>> {
    ...
    for p in paths {
        ...
        match std::fs::symlink_metadata(p) {
            ...
            Ok(_) => {
                // Already exists as a regular file/dir - leave it alone.
            }
            Err(_) => {
                std::fs::write(p, CANARY_CONTENT)
        ...
        let canon = p
            .canonicalize()
            .with_context(|| format!("canonicalizing honeypot path {}", p.display()))?;
        set.insert(canon);
    }
    Ok(set)
}
...
pub fn is_honeypot(honeypots: &HashSet<PathBuf>, path: &Path) -> bool {
    match path.canonicalize() {
        ...
        // File may have been deleted/renamed by the very event we're
        // handling; fall back to a direct comparison.
